--[[
    ...
    function Library:Notifications(Name: string)
    function Library:Notification(Text: string, Duration: number, Color: Color3, Icon: table)
    function Library:KeybindList()

    function Page:PlayerList(Data: table
        Callback/callback: function(PlayerName: string, Priority: string),
        ViewCallback/viewcallback: function(Player: Player, IsViewing: boolean),
        TpCallback/tpcallback: function(Player: Player)
    )
]]
...
local Library
    ...
    Library = {
        ...
        Theme = {
            ...
            ["Friendly"] = FromRGB(38, 89, 140),
            ["Priority"] = FromRGB(140, 0, 2)
        },
    ...
    local Instances = { } do
        ...
        Instances.Connect = function(self, Event, Callback, Name)
            ...
            if Event == "MouseButton1Down" or Event == "MouseButton2Down" then
                local ButtonType = (Event == "MouseButton1Down") and Enum.UserInputType.MouseButton1 or Enum.UserInputType.MouseButton2
                if self.Instance.InputBegan then
                    return Library:Connect(self.Instance.InputBegan, function(Input)
                        if Library.CurrentDrag or Library.IsSliding then return end
                        if Input.UserInputType == ButtonType or Input.UserInputType == Enum.UserInputType.Touch then
                            Callback(Input)
                        end
                    end, Name)
                end
            elseif Event == "MouseButton1Click" or Event == "MouseButton2Click" then
                local ButtonType = (Event == "MouseButton1Click") and Enum.UserInputType.MouseButton1 or Enum.UserInputType.MouseButton2
                if self.Instance.InputEnded then
                    return Library:Connect(self.Instance.InputEnded, function(Input)
                        if Library.CurrentDrag or Library.IsSliding then return end
                        if Input.UserInputType == ButtonType or Input.UserInputType == Enum.UserInputType.Touch then
                            Callback(Input)
                        end
        ...
        Instances.MakeDraggable = function(self)
            ...
            self:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    if Library.IsResizing then return end
                    if Library.CurrentDrag and Library.CurrentDrag ~= Gui then
                        return
                    end
                    Library.CurrentDrag = Gui
                    Dragging = true
                    ActiveInput = Input
            ...
            Library:Connect(UserInputService.InputChanged, function(Input)
                if not Dragging then return end
                if Library.CurrentDrag and Library.CurrentDrag ~= Gui then return end
        ...
        Instances.MakeResizeable = function(self, Minimum, Maximum)
            ...
            local OverlayInner = Instances:Create("Frame", {
                Parent = Overlay.Instance,
                Name = "\0",
            ...
            Instances:Create("UICorner", {
                Parent = OverlayInner.Instance,
                CornerRadius = UDimNew(0, 8)
            })

            local SizeLabel = Instances:Create("TextLabel", {
                Parent = Overlay.Instance,
                Name = "\0",
                Size = UDim2New(1, 0, 1, 0),
                BackgroundTransparency = 1,
                Text = "",
                TextColor3 = FromRGB(120, 120, 128),
                TextSize = 13,
            ...
            local ClearOverlayTweens = function()
                for _, T in OverlayTweens do
            ...
            local ShowOverlay = function()
                ClearOverlayTweens()
                Overlay.Instance.Visible = true
                Overlay.Instance.BackgroundTransparency = 1
                OverlayInner.Instance.BackgroundTransparency = 1
                SizeLabel.Instance.TextTransparency = 1
                local T1 = TweenService:Create(Overlay.Instance, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {BackgroundTransparency = 0})
                local T2 = TweenService:Create(OverlayInner.Instance, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {BackgroundTransparency = 0})
                local T3 = TweenService:Create(SizeLabel.Instance, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {TextTransparency = 0.2})
                OverlayTweens = {T1, T2, T3}
                T1:Play()
                T2:Play()
                T3:Play()
                task.delay(0.1, function()
                    if Resizing then
                        ...
                        OverlayInner.Instance.BackgroundTransparency = 0
                    end
                end)
            end

            local HideOverlay = function()
                ClearOverlayTweens()
                local T1 = TweenService:Create(Overlay.Instance, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {BackgroundTransparency = 1})
                local T2 = TweenService:Create(OverlayInner.Instance, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {BackgroundTransparency = 1})
                local T3 = TweenService:Create(SizeLabel.Instance, TweenInfo.new(0.15, Enum.EasingStyle.Quad), {TextTransparency = 1})
                OverlayTweens = {T1, T2, T3}
                T1:Play()
                T2:Play()
                T3:Play()
            ...
            Handle:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    if Library.CurrentDrag or Library.IsResizing then
                        return
                    end
                    Library.CurrentDrag = Gui
            ...
            Library:Connect(UserInputService.InputChanged, function(Input)
                ...
                local NewW = StartSize.X.Offset + (Input.Position.X - StartPos.X)
                local NewH = StartSize.Y.Offset + (Input.Position.Y - StartPos.Y)

                NewW = MathClamp(NewW, Minimum.X, Maximum and Maximum.X or ViewportX)
                NewH = MathClamp(NewH, Minimum.Y, Maximum and Maximum.Y or ViewportY)

                Gui.Size = UDim2New(StartSize.X.Scale, NewW, StartSize.Y.Scale, NewH)

                SizeLabel.Instance.Text = StringFormat("%d × %d", NewW, NewH)
            end)
    ...
    -- Draggable handle is provided by Library:Notifications() (watermark-style body)
    ...
    -- 批量淡入/淡出：一次遍历收集所有属性，统一设置初始状态，只创建必要的 Tween
    -- 避免为每个属性单独创建 Connection 和 coroutine
    Library.BatchFade = function(self, RootInstance, Visibility, Speed)
        Speed = Speed or Library.Tween.Time
        local Descendants = RootInstance:GetDescendants()
        Descendants[#Descendants + 1] = RootInstance

        local TweenInfoObj = TweenInfo.new(Speed, Library.Tween.Style, Library.Tween.Direction)
        local LastTween = nil
        local Count = 0

        for _, Value in Descendants do
            local Properties = self:GetTransparencyPropertyFromItem(Value)
            if not Properties then
                continue
            end

            for _, Property in Properties do
                local OldTransparency = Value[Property]
                -- 瞬间设置到起始状态
                Value[Property] = Visibility and 1 or OldTransparency

                -- 创建 Tween（不经过 FadeItem，避免额外的 Connection）
                local NewTween = TweenService:Create(Value, TweenInfoObj, {
                ...
                if not Visibility then
                    ...
                    NewTween.Completed:Once(function(State)
                        if State == Enum.PlaybackState.Completed then
                            Value[Property] = OldTransparency
                        end
                    end)
                end
    ...
    Library.FadeItem = function(self, Item, Property, Visibility, Speed)
        ...
        -- 使用 :Once() 替代 Library:Connect，避免 Connection 累积泄漏
    ...
    Library.LoadConfig = function(self, Config)
        ...
        if not DecodeSuccess or type(Decoded) ~= "table" then
            Library:Notification("Failed to load config: invalid format", 5, Color3.fromRGB(255, 0, 0))
            return
        end
    ...
    Library.ChangeTheme = function(self, Theme, Color)
        ...
        for _, Callback in self.ThemeCallbacks do
            Callback(Theme, Color)
        end
    end

    Library.AddThemeCallback = function(self, Callback)
        TableInsert(self.ThemeCallbacks, Callback)
        return Callback
    end

    Library.RemoveThemeCallback = function(self, Callback)
        for Index, Value in self.ThemeCallbacks do
            if Value == Callback then
                TableRemove(self.ThemeCallbacks, Index)
                break
            end
        end
    end
    ...
    Library.Watermark = function(self, Name)
        ...
        Watermark.UIScale = Instances:Create("UIScale", {
            Parent = Items["Watermark"].Instance,
            Scale = 1.0
        })
    ...
    Library.Notifications = function(self, Name)
        local Notifications = { } 
        local BodyWidth = 0

        local Items = { } do 
            Items["Notifications"] = Instances:Create("Frame", {
                Parent = Library.Holder.Instance,
                Size = UDim2New(0, 0, 0, 20),
                Name = "\0",
                ...
                BorderSizePixel = 2,
                BackgroundColor3 = FromRGB(15, 15, 20)
            })  Items["Notifications"]:AddToTheme({BackgroundColor3 = "Background", BorderColor3 = "Border"})

            Items["Notifications"]:MakeDraggable()
            
            Instances:Create("UIStroke", {
                Parent = Items["Notifications"].Instance,
                ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                LineJoinMode = Enum.LineJoinMode.Miter,
                Name = "\0",
                Color = FromRGB(27, 27, 32)
            ...
            -- Compute a fixed width from the title text so the body never auto-grows with notifications
            BodyWidth = Items["Title"].Instance.TextBounds.X + 14 -- 5px padding each side + 2px border each side
            
            Items["Notifications"].Instance.Size = UDim2New(0, BodyWidth, 0, 20)
            ...
            -- Reparent the notification list holder into this body so dragging moves every notification
            Library.NotifHolder.Instance.Parent = Items["Notifications"].Instance
            Library.NotifHolder.Instance.AnchorPoint = Vector2New(0, 0)
            Library.NotifHolder.Instance.Position = UDim2New(0, 0, 0, 22)
            Library.NotifHolder.Instance.Size = UDim2New(0, BodyWidth, 0, 0)
            Library.NotifHolder.Instance.AutomaticSize = Enum.AutomaticSize.Y
            Library.NotifHolder.Instance.Active = false
            Library.NotifHolder.Instance.BackgroundTransparency = 1
        end

        function Notifications:SetVisibility(Bool)
            local Frame = Items["Notifications"].Instance
            local StrokeVis = Bool and 0 or 1

            -- Keep Visible=true so notifications keep showing; only fade the title bar visuals
            Frame.Visible = true
            Frame.Size = UDim2New(0, BodyWidth, 0, Bool and 20 or 0)
            Frame.BackgroundTransparency = Bool and 0 or 1
            Items["Title"].Instance.TextTransparency = Bool and 0 or 1
            Items["AccentLine"].Instance.BackgroundTransparency = Bool and 0 or 1
            Library.NotifHolder.Instance.Position = UDim2New(0, 0, 0, Bool and 22 or 0)

            -- Border stroke on the body
            for _, Child in Frame:GetChildren() do
                if Child ~= Library.NotifHolder.Instance and Child:IsA("UIStroke") then
                    Child.Transparency = StrokeVis
                end
            end

            -- Stroke on the title text
            for _, Child in Items["Title"].Instance:GetChildren() do
        ...
        Notifications.UIScale = Instances:Create("UIScale", {
            Parent = Items["Notifications"].Instance,
            Scale = 1.0
        })
    ...
    Library.KeybindList = function(self)
        ...
        KeybindList.UIScale = Instances:Create("UIScale", {
            Parent = Items["KeybindList"].Instance,
            Scale = 1.0
        })
    ...
    Library.CreateColorpicker = function(self, Data)
        ...
        function Colorpicker:SetOpen(Bool)
            ...
            local TweenInfoObj = TweenInfo.new(Data.FadeSpeed or Library.Tween.Time, Library.Tween.Style, Library.Tween.Direction)
            local LastTween = nil
            ...
            for Index, Value in Descendants do 
                ...
                for _, Property in ValueIndex do 
                    local OldTransparency = Value[Property]
                    Value[Property] = Bool and 1 or OldTransparency

                    local NewTween = TweenService:Create(Value, TweenInfoObj, {
                        [Property] = Bool and OldTransparency or 1
                    })
                    NewTween:Play()
        ...
        function Colorpicker:Toggle(Data)
            return self.Section:Toggle(Data)
        end
    ...
    Library.CreateKeybind = function(self, Data)
        ...
        local Items = { } do 
            ...
            Items["Reverse Hold"] = Instances:Create("TextButton", {
                Parent = Items["Window"].Instance,
                FontFace = Library.Font,
                TextColor3 = FromRGB(215, 215, 215),
                BorderColor3 = FromRGB(0, 0, 0),
                ...
                BorderSizePixel = 0,
                BackgroundTransparency = 1,
                Position = UDim2New(0, 1, 0, 45),
                Size = UDim2New(1, 0, 0, 15),
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255)
            })  Items["Reverse Hold"]:AddToTheme({TextColor3 = "Text"})
            
            Instances:Create("UIStroke", {
                Parent = Items["Reverse Hold"].Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
                Name = "\0"
            }):AddToTheme({Color = "Text Border"})
        end
        ...
        local Modes = {
            ...
            ["Hold"] = Items["Hold"],
            ["Always"] = Items["Always"],
            ["Reverse Hold"] = Items["Reverse Hold"]
        }
        ...
        function Keybind:Set(Key)
           ...
           elseif TableFind({"Toggle", "Hold", "Always", "Reverse Hold"}, Key) then 
                Keybind.Mode = Key
        ...
        function Keybind:SetMode(Mode)
            ...
            if Keybind.Mode == "Always" or Keybind.Mode == "Reverse Hold" then 
        ...
        function Keybind:Press(Bool)
            ...
            elseif Keybind.Mode == "Reverse Hold" then
                Keybind.Toggled = not Bool
            ...
            if Data.OnPress then
                Library:SafeCall(Data.OnPress, Keybind.Mode, Keybind.Toggled)
            end
        ...
        Items["Reverse Hold"]:Connect("MouseButton1Down", function()
            Keybind.Mode = "Reverse Hold"
            Keybind:SetMode("Reverse Hold")
        end)
    ...
    Library.Window = function(self, Data)
        ...
        local Window = {
            ...
            Size = Data.Size or Data.size or UDim2New(0, 550, 0, 380),
    ...
    Library.Page = function(self, Data)
        ...
        function Page:Turn(Bool)
            ...
            local LastTween = Library:BatchFade(Items["Page"].Instance, Bool, Page.Window.FadeSpeed or 0.5)
    ...
    Library.Pages.SubPage = function(self, Data)
        ...
        function SubPage:Turn(Bool)
            if Debounce or SubPage.Active == Bool then 
                return 
            end
            ...
            if Bool then 
                ...
                Items["Icon"]:Tween(nil, {TextColor3 = Library.Theme.Accent, TextTransparency = 0})
                Items["Hide"].Instance.Visible = true

                Items["Icon"]:ChangeItemTheme({TextColor3 = "Accent"})

                Items["Inactive"].Instance.Size = UDim2New(1, 0, 1, 1)
            else
                Items["Icon"]:Tween(nil, {TextColor3 = Library.Theme.Text, TextTransparency = 0.35})
                Items["Hide"].Instance.Visible = false

                Items["Icon"]:ChangeItemTheme({TextColor3 = "Text"})
                Items["Inactive"].Instance.Size = UDim2New(1, 0, 1, -2)
            end

            local LastTween = Library:BatchFade(Items["Subtab"].Instance, Bool, SubPage.Window.FadeSpeed or 0.5)
    ...
    Library.Pages.MultiSection = function(self, Data)
        ...
        for Index, Value in MultiSection.Sections do 
            ...
            function NewSection:Turn(Bool)
                ...
                local LastTween = Library:BatchFade(SubItems["Content"].Instance, Bool, MultiSection.Window.FadeSpeed or 0.5)
    ...
    -- ===== PlayerList（Bbot 原版移植） =====
    Library.Pages.PlayerList = function(self, Data)
        Data = Data or {}

        local Page = self

        -- 等待 Page 初始淡入动画完成，避免销毁正在动画中的元素导致 Debounce 卡死
        if Page.Active then
            task.wait((Page.Window and Page.Window.FadeSpeed or 0.5) + 0.05)
        end
        ...
        local Cfg = {
            ...
            ViewCallback = Data.ViewCallback or Data.viewcallback or function() end,
            TpCallback = Data.TpCallback or Data.tpcallback or function() end,
            Viewing = false,
            ViewTarget = nil,
        }
        ...
        -- 销毁 Page 上所有子元素（仿 Bbot 的 self.Items.Left:Destroy() / self.Items.Right:Destroy()）
        local PageRoot = Page.Elements["Page"]
        for _, Child in PageRoot.Instance:GetChildren() do
            Child:Destroy()
        end
        Page.ColumnsData = {}

        -- 在 Page 上创建 Flex 布局（仿 Bbot 原版）
        Instances:Create("UIListLayout", {
            Parent = PageRoot.Instance,
            FillDirection = Enum.FillDirection.Horizontal,
            HorizontalFlex = Enum.UIFlexAlignment.Fill,
            Padding = UDimNew(0, 4),
            SortOrder = Enum.SortOrder.LayoutOrder,
        ...
        local Items = {} do
            -- Column（仿 Bbot：Size = 0,100,0,100，靠 Flex 撑满）
            Items.Column = Instances:Create("Frame", {
                Parent = PageRoot.Instance,
                BackgroundTransparency = 1,
                Name = "\0",
                BorderColor3 = FromRGB(0, 0, 0),
            ...
            Items.Outline = Instances:Create("Frame", {
                ...
                Parent = Items.Column.Instance,
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, 0, 1, -92),
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Outline,
            }) Items.Outline:AddToTheme({BackgroundColor3 = "Outline"})

            -- PriorityHolder（标题栏外框，仿 Bbot：Position = 0,1,0,19 / Size = 1,-2,0,18）
            Items.PriorityHolder = Instances:Create("Frame", {
                Parent = Items.Outline.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 19),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -2, 0, 18),
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Outline,
            }) Items.PriorityHolder:AddToTheme({BackgroundColor3 = "Outline"})
            ...
            Items.Background = Instances:Create("Frame", {
                ...
                Size = UDim2New(1, -2, 1, -2),
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(36, 36, 36),
            ...
            -- 标题 "Name"
            local TitleName = Instances:Create("TextLabel", {
                LayoutOrder = 1,
                FontFace = Library.Font,
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255),
            }) TitleName:AddToTheme({TextColor3 = "Text"})

            Instances:Create("UIStroke", {
                Parent = TitleName.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})
            ...
            -- 标题 "Team"
            local TitleTeam = Instances:Create("TextLabel", {
                LayoutOrder = 2,
                FontFace = Library.Font,
                TextColor3 = Library.Theme.Text,
                BorderColor3 = FromRGB(0, 0, 0),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255),
            }) TitleTeam:AddToTheme({TextColor3 = "Text"})

            Instances:Create("UIStroke", {
                Parent = TitleTeam.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})

            -- 标题 "Priority"
            local TitlePriority = Instances:Create("TextLabel", {
                LayoutOrder = 3,
                FontFace = Library.Font,
                TextColor3 = Library.Theme.Text,
                BorderColor3 = FromRGB(0, 0, 0),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255),
            }) TitlePriority:AddToTheme({TextColor3 = "Text"})

            Instances:Create("UIStroke", {
                Parent = TitlePriority.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})
            ...
            -- ScrollingFrame（仿 Bbot：Position = 0,2,0,38 / Size = 1,-4,1,-38）
            -- 背景绑定 Inline 主题色，使列表主体跟随 Inline 设置而非 Outline
            Items.ScrollingFrame = Instances:Create("ScrollingFrame", {
                ...
                ScrollBarThickness = 3,
                Parent = Items.Outline.Instance,
                Size = UDim2New(1, -4, 1, -38),
                BorderColor3 = FromRGB(0, 0, 0),
                BackgroundColor3 = Library.Theme.Inline,
                TopImage = Library:GetImage("Scrollbar"),
                Position = UDim2New(0, 2, 0, 38),
                BackgroundTransparency = 0,
                BottomImage = Library:GetImage("Scrollbar"),
                BorderSizePixel = 0,
                CanvasSize = UDim2New(0, 0, 0, 0),
            }) Items.ScrollingFrame:AddToTheme({ScrollBarImageColor3 = "Accent", BackgroundColor3 = "Inline"})
            ...
            -- Refresh 按钮（仿 Bbot：AnchorPoint = 1,0 / Position = 1,-1,0,1 / Size = 0,58,0,18）
            Items.Refresh = Instances:Create("TextButton", {
                Active = false,
                BorderColor3 = FromRGB(0, 0, 0),
                AnchorPoint = Vector2New(1, 0),
                Parent = Items.Outline.Instance,
                Name = "\0",
                Position = UDim2New(1, -1, 0, 1),
                Size = UDim2New(0, 58, 0, 18),
                Selectable = false,
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Outline,
            }) Items.Refresh:AddToTheme({BackgroundColor3 = "Outline"})

            local RefreshInline = Instances:Create("Frame", {
                Parent = Items.Refresh.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -2, 1, -2),
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Inline,
            }) RefreshInline:AddToTheme({BackgroundColor3 = "Inline"})

            local RefreshBg = Instances:Create("Frame", {
                Parent = RefreshInline.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -2, 1, -2),
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(36, 36, 36),
            }) RefreshBg:AddToTheme({BackgroundColor3 = "Element"})

            local RefreshText = Instances:Create("TextLabel", {
                FontFace = Library.Font,
                TextColor3 = Library.Theme.Text,
                BorderColor3 = FromRGB(0, 0, 0),
                Text = "Refresh",
                Parent = RefreshBg.Instance,
                Name = "\0",
                Size = UDim2New(1, 0, 1, 0),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255),
            }) RefreshText:AddToTheme({TextColor3 = "Text"})

            Instances:Create("UIStroke", {
                Parent = RefreshText.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})
            ...
            -- 搜索框 Holder（仿 Bbot：Position = 0,1,0,1 / Size = 1,-60,0,18）
            Items.Holder = Instances:Create("Frame", {
                BorderColor3 = FromRGB(0, 0, 0),
                Parent = Items.Outline.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                Size = UDim2New(1, -60, 0, 18),
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Outline,
            }) Items.Holder:AddToTheme({BackgroundColor3 = "Outline"})
            ...
            local HolderInline = Instances:Create("Frame", {
                Parent = Items.Holder.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                BorderColor3 = FromRGB(0, 0, 0),
                ...
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Inline,
            }) HolderInline:AddToTheme({BackgroundColor3 = "Inline"})

            local HolderBg = Instances:Create("Frame", {
                Parent = HolderInline.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -2, 1, -2),
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(36, 36, 36),
            }) HolderBg:AddToTheme({BackgroundColor3 = "Element"})

            -- PlayerSelection（底部选中面板）
            Items.PlayerSelection = Instances:Create("Frame", {
                Name = "\0",
                Parent = Items.Column.Instance,
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, 0, 0, 88),
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Outline,
            }) Items.PlayerSelection:AddToTheme({BackgroundColor3 = "Outline"})

            -- PlayerSelection 内层 Inline（跟随主题）
            local PSInline = Instances:Create("Frame", {
                Parent = Items.PlayerSelection.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -2, 1, -2),
                BorderSizePixel = 0,
                BackgroundColor3 = Library.Theme.Inline,
            }) PSInline:AddToTheme({BackgroundColor3 = "Inline"})

            -- PlayerSelection 内层 Element 背景（跟随主题）
            local PSBg = Instances:Create("Frame", {
                Parent = PSInline.Instance,
                Name = "\0",
                Position = UDim2New(0, 1, 0, 1),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -2, 1, -2),
            ...
            }) PSBg:AddToTheme({BackgroundColor3 = "Element"})

            -- 头像（仿 Bbot：AnchorPoint = 0,0.5 / Position = 0,8,0.5,0 / Size = 0,64,0,64）
            Items.ImageProfile = Instances:Create("ImageLabel", {
                BorderColor3 = FromRGB(0, 0, 0),
                Parent = Items.PlayerSelection.Instance,
                Name = "\0",
                AnchorPoint = Vector2New(0, 0.5),
            ...
            -- 左侧信息标签区（仿 Bbot：Position = 0,84,0,8 / Size = 0,100,0,1）
            Items.InfoElements = Instances:Create("Frame", {
                Parent = Items.PlayerSelection.Instance,
                Name = "\0",
                BackgroundTransparency = 1,
                Position = UDim2New(0, 84, 0, 8),
                BorderColor3 = FromRGB(0, 0, 0),
            ...
            Instances:Create("UIListLayout", {
                Parent = Items.InfoElements.Instance,
                Padding = UDimNew(0, 3),
                SortOrder = Enum.SortOrder.LayoutOrder,
            })

            -- 手动创建 Label（当前库的 Label 没有 Set 方法，手动实现）
            local function MakeLabel(Text)
                ...
                Instances:Create("UIStroke", {
                    Parent = TextLabel.Instance,
                    LineJoinMode = Enum.LineJoinMode.Miter,
                }):AddToTheme({Color = "Text Border"})

                return { Set = function(text) TextLabel.Instance.Text = text end }
            end

            Items.NameLabel = MakeLabel("Name: ???")
            Items.DisplayNameLabel = MakeLabel("DisplayName: ???")
            Items.TeamLabel = MakeLabel("Team: ???")

            -- 右侧优先级下拉栏区（仿 Bbot：AnchorPoint = 1,0 / Position = 1,-4,0,8 / Size = 0,150,0,1）
            Items.PriorityElements = Instances:Create("Frame", {
                BorderColor3 = FromRGB(0, 0, 0),
                AnchorPoint = Vector2New(1, 0),
                Parent = Items.PlayerSelection.Instance,
                BackgroundTransparency = 1,
                Position = UDim2New(1, -4, 0, 8),
                Name = "\0",
                Size = UDim2New(0, 150, 0, 1),
                BorderSizePixel = 0,
                BackgroundColor3 = FromRGB(255, 255, 255),
            })

            Instances:Create("UIListLayout", {
                Parent = Items.PriorityElements.Instance,
                Padding = UDimNew(0, 3),
                SortOrder = Enum.SortOrder.LayoutOrder,
            })

            -- 使用当前库的 Dropdown（通过 FakeSection 适配 API）
            local FakeSection = setmetatable({
                Window = Page.Window,
                Page = Page,
                Elements = { ["Content"] = Items.PriorityElements },
            }, Library.Sections)

            Items.PriorityDropdown = FakeSection:Dropdown({
                Name = "Priority",
                Items = {"Friendly", "Priority", "Neutral"},
            ...
            -- View 按钮（toggle 模式，使用库标准 Button 样式）
            local ViewButtonObj = FakeSection:Button({
                Name = "View",
                Callback = function()
                    if not Cfg.Selected or not Cfg.Selected.Player then
                        return
                    end

                    Cfg.Viewing = not Cfg.Viewing
                    local Player = Cfg.Selected.Player

                    if Cfg.Viewing then
                        Cfg.ViewTarget = Player
                        Library:SafeCall(Cfg.ViewCallback, Player, true)
                    else
                        Cfg.ViewTarget = nil
                        Library:SafeCall(Cfg.ViewCallback, Player, false)
                    end
                end,
            })

            -- 找到 View 按钮实例，覆盖 Press 实现 toggle 视觉状态
            local ViewBtnInstance, ViewTextInstance
            for _, child in Items.PriorityElements.Instance:GetChildren() do
                if child:IsA("TextButton") then
                    local label = child:FindFirstChildWhichIsA("TextLabel")
                    if label and label.Text == "View" then
                        ViewBtnInstance = child
                        ViewTextInstance = label
                        break
                    end
                end
            end

            if ViewBtnInstance then
                ViewButtonObj.Press = function()
                    Library:SafeCall(ViewButtonObj.Callback)

                    if Cfg.Viewing then
                        -- 观看中：保持高亮
                        ViewTextInstance.TextColor3 = Library.Theme.Accent
                        ViewBtnInstance.BackgroundColor3 = Library.Theme.Accent
                    else
                        -- 取消观看：闪烁后恢复
                        ViewTextInstance.TextColor3 = Library.Theme.Accent
                        ViewBtnInstance.BackgroundColor3 = Library.Theme.Accent
                        task.wait(0.1)
                        ViewTextInstance.TextColor3 = Library.Theme.Text
                        ViewBtnInstance.BackgroundColor3 = Library.Theme.Element
                    end
                end
            end

            -- TP 按钮（使用库标准 Button 样式，默认 Press 行为即可）
            FakeSection:Button({
                Name = "TP",
                Callback = function()
                    if not Cfg.Selected or not Cfg.Selected.Player then
                    ...
                    Library:SafeCall(Cfg.TpCallback, Cfg.Selected.Player)
                end,
            })
        end
        ...
        function Cfg.AddPlayer(player)
            ...
            SeperateData.Background = Instances:Create("TextButton", {
                Active = false,
                BorderColor3 = FromRGB(0, 0, 0),
                Parent = Items.ScrollingFrame.Instance,
                Name = "\0",
            ...
            Instances:Create("UIListLayout", {
                FillDirection = Enum.FillDirection.Horizontal,
                HorizontalFlex = Enum.UIFlexAlignment.Fill,
                Parent = SeperateData.Background.Instance,
                SortOrder = Enum.SortOrder.LayoutOrder,
                VerticalFlex = Enum.UIFlexAlignment.Fill,
            })

            -- Name 列（AutomaticSize.X，仿 Bbot）
            SeperateData.Name = Instances:Create("TextLabel", {
                LayoutOrder = 1,
                FontFace = Library.Font,
                TextColor3 = Library.Theme.Text,
                BorderColor3 = FromRGB(0, 0, 0),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255),
            }) SeperateData.Name:AddToTheme({TextColor3 = "Text"})

            Instances:Create("UIStroke", {
                Parent = SeperateData.Name.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})
            ...
            -- Team 列（AutomaticSize.X，仿 Bbot）
            SeperateData.Team = Instances:Create("TextLabel", {
                LayoutOrder = 2,
                FontFace = Library.Font,
                TextColor3 = Library.Theme.Text,
                BorderColor3 = FromRGB(0, 0, 0),
                ...
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255),
            }) SeperateData.Team:AddToTheme({TextColor3 = "Text"})

            Instances:Create("UIStroke", {
                Parent = SeperateData.Team.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})

            -- 监听 Team 变化
            Library:Connect(player:GetPropertyChangedSignal("Team"), function()
                if SeperateData.Team and SeperateData.Team.Instance then
                    SeperateData.Team.Instance.Text = GetTeamName(player)
                    if Cfg.Selected == SeperateData then
                        Items.TeamLabel.Set("Team: " .. GetTeamName(player))
                    end
                end
            end)

            -- Priority 列（AutomaticSize.X，右对齐，仿 Bbot）
            local IsLocal = player.Name == Players.LocalPlayer.Name
            local InitPriority = IsLocal and "LocalPlayer" or "Neutral"
            SeperateData.Priority = Instances:Create("TextLabel", {
                LayoutOrder = 3,
                FontFace = Library.Font,
                TextColor3 = IsLocal and FromRGB(119, 119, 119) or Library.Theme[Colors["Neutral"]],
                BorderColor3 = FromRGB(0, 0, 0),
                Text = InitPriority,
                Parent = SeperateData.Background.Instance,
                Name = "\0",
            ...
            Instances:Create("UIStroke", {
                Parent = SeperateData.Priority.Instance,
                LineJoinMode = Enum.LineJoinMode.Miter,
            }):AddToTheme({Color = "Text Border"})
            ...
            -- 点击选中（仿 Bbot 原版）
            SeperateData.Background:Connect("MouseButton1Down", function()
                if SeperateData.Priority.Instance.Text == "LocalPlayer" then
                    return
                end
                ...
                if Old then
                    Old.Name.Instance.TextColor3 = Library.Theme.Text
                    Old.Team.Instance.TextColor3 = Library.Theme.Text
                end

                SeperateData.Name.Instance.TextColor3 = Library.Theme.Accent
                SeperateData.Team.Instance.TextColor3 = Library.Theme.Accent

                task.spawn(function()
                    local Success, Image = pcall(function()
                        return Players:GetUserThumbnailAsync(player.UserId, Enum.ThumbnailType.AvatarThumbnail, Enum.ThumbnailSize.Size150x150)
                    end)
                    if Success then
                        Items.ImageProfile.Instance.Image = Image
                    end
                end)
                ...
                task.wait()
                Items.PriorityDropdown:Set(SeperateData.Priority.Instance.Text)
            end)

            Cfg.Players[player.Name] = SeperateData
        end

        -- ModifyPriority（仿 Bbot 原版，Priority 文字颜色平滑过渡）
        function Cfg.ModifyPriority(Priority, Target)
            local Path = Target or Cfg.Selected
            if Path then
                Path.Priority.Instance.Text = Priority
                Path.Priority:Tween(TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                    TextColor3 = Library.Theme[Colors[Priority]] or Library.Theme.Text
                })
                if Cfg.Callback and Path.Player then
                    Library:SafeCall(Cfg.Callback, Path.Player.Name, Priority)
                end
            end
        end

        -- GetPriority（仿 Bbot 原版）
        function Cfg.GetPriority(Player)
            local Data = Cfg.Players[Player]
            return Data and Data.Priority.Instance.Text or nil
        end
        ...
        -- Refresh 按钮点击（仿 Bbot 原版，直接遍历 Cfg.Players 避免重复 GetPlayers）
        Items.Refresh:Connect("MouseButton1Down", function()
            for _, Data in Cfg.Players do
                if Data.Player ~= Players.LocalPlayer then
                    Cfg.ModifyPriority("Neutral", Data)
                end
            end
        end)

        -- 搜索过滤（仿 Bbot 原版，修复 return->continue bug + 缓存 lower）
        Library:Connect(Items.Search.Instance:GetPropertyChangedSignal("Text"), function()
            local Text = string.lower(Items.Search.Instance.Text)

            for Name, Data in Cfg.Players do
                if not Data or not Data.Background then
                    continue
                end

                if string.find(string.lower(Name), Text, 1, true) then
                    Data.Background.Instance.Visible = true
                else
                    Data.Background.Instance.Visible = false
                end
            end
        end)
        ...
        -- 主题变化时刷新优先级颜色（Friendly / Priority / Text 改动后同步更新，平滑过渡）
        local ThemeCallback = Library:AddThemeCallback(function(ThemeKey)
            if Colors[ThemeKey] or ThemeKey == "Text" then
                for _, Data in Cfg.Players do
                    if Data.Priority and Data.Priority.Instance then
                        local Priority = Data.Priority.Instance.Text
                        if Priority ~= "LocalPlayer" then
                            Data.Priority:Tween(TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                                TextColor3 = Library.Theme[Colors[Priority]] or Library.Theme.Text
                            })
                        end
                    end
        ...
        -- 导出方法到 Page
        Page.GetPriority = function(_, Name) return Cfg.GetPriority(Name) end
        Page.SetPriority = function(_, Name, Priority)
            local Data = Cfg.Players[Name]
            if Data then
                Cfg.ModifyPriority(Priority, Data)
            end
        end
        Page.RefreshPlayerList = function()
            for _, Data in Cfg.Players do
                if Data.Player ~= Players.LocalPlayer then
                    Cfg.ModifyPriority("Neutral", Data)
                end
    ...
    Library.Sections.Toggle = function(self, Data)
        ...
        function Toggle:Keybind(Data)
            ...
            local UserCallback = Data.Callback or Data.callback or function() end
            ...
            local Keybind = {
                ...
                OnPress = function(Mode, Toggled)
                    if Mode == "Hold" or Mode == "Reverse Hold" then
                        Toggle:Set(Toggled)
                    else
                        Toggle:Set()
                    end
    ...
    Library.Sections.Slider = function(self, Data)
        ...
        local Slider = {
            ...
            Callback = Data.Callback or Data.callback or function() end,
            OnRelease = Data.OnRelease or Data.onrelease or nil,
        ...
        Library:Connect(UserInputService.InputEnded, function(Input)
            if not Slider.Sliding then return end

            if Slider.ActiveInput == Input
            or (Slider.ActiveInput and Slider.ActiveInput.UserInputType == Enum.UserInputType.MouseButton1
                and Input.UserInputType == Enum.UserInputType.MouseButton1) then
                Slider.Sliding = false
                Slider.ActiveInput = nil
                Library.IsSliding = false

                if Slider.OnRelease then
                    Library:SafeCall(Slider.OnRelease, Slider.Value)
                end
            end
        end)
        ...
        Library:Connect(UserInputService.InputChanged, function(Input)
            ...
            if Slider.ActiveInput.UserInputType == Enum.UserInputType.MouseButton1 then
                if Input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
            elseif Slider.ActiveInput.UserInputType == Enum.UserInputType.Touch then
                if Input ~= Slider.ActiveInput then return end
            else
                return
            end
    ...
    Library.Sections.Dropdown = function(self, Data)
        ...
        function Dropdown:Refresh(List)
            local OldOptions = {}
            for Index, Value in Dropdown.Options do 
                table.insert(OldOptions, Value.Name)
            end

            for _, Name in OldOptions do 
        ...
        function Dropdown:SetOpen(Bool)
            ...
            local TweenInfoObj = TweenInfo.new(Dropdown.Window.FadeSpeed or Library.Tween.Time, Library.Tween.Style, Library.Tween.Direction)
            local LastTween = nil
    ...
    Library.Sections.Colorpicker = function(self, Data)
        ...
        local Label = Library.Sections.Label(self, {
            Name = Data.Name or Data.name or "Colorpicker",
            Alignment = Data.Alignment or Data.alignment or "Left",
        })

        return Label:Colorpicker(Data)
    end
    ...
    Library.CreateSettingsPage = function(self, Window, Watermark, KeybindList, Notifications)
        ...
        do -- Settings Tab
            ...
            SettingsSection:Toggle({Name = "Notifications Holder", Flag = "Notifications Holder", Default = false, Callback = function(Value)
                Notifications:SetVisibility(Value)
            end})
            ...
            SettingsSection:Slider({Name = "Menu Scale", Min = 0.5, Max = 2, Default = 1, Decimals = 0.05, Flag = "Menu Scale", OnRelease = function(Value)
                if Library.UIScale then
                    Library.UIScale.Instance.Scale = Value
                end
            end})
        
            SettingsSection:Slider({Name = "Watermark Scale", Min = 0.5, Max = 2, Default = 1, Decimals = 0.05, Flag = "Watermark Scale", OnRelease = function(Value)
                if Watermark.UIScale then
                    Watermark.UIScale.Instance.Scale = Value
                end
            end})
        
            SettingsSection:Slider({Name = "Notifications Scale", Min = 0.5, Max = 2, Default = 1, Decimals = 0.05, Flag = "Notifications Scale", OnRelease = function(Value)
                if Notifications.UIScale then
                    Notifications.UIScale.Instance.Scale = Value
                end
            end})
        
            SettingsSection:Slider({Name = "Keybinds Scale", Min = 0.5, Max = 2, Default = 1, Decimals = 0.05, Flag = "Keybinds Scale", OnRelease = function(Value)
                if KeybindList.UIScale then
                    KeybindList.UIScale.Instance.Scale = Value
                end
            end})
